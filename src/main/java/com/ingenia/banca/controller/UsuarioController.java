...
public class UsuarioController {
    ...
    public ResponseEntity<?> findOneUser(@ApiParam("Clave primaria del usuario en formato nif: [[A-H][J-N][P-S]UVW][0-9]{7}[0-9A-J]")@PathVariable String nif) {
        log.debug("Rest request a Usuario with id: "+nif);
        Optional<Usuario> userOpt;
        Map<String, Object> response = new HashMap<>();
        try{
            userOpt = userService.findUsuarioByNif(nif);
        ...
        response.put("mensaje", "El cliente ID: ".concat(nif.toString().concat(" no existe en la base de datos!")));
